...
Voici les modalités de la variabme `Goods` qui signifie 
`Marchandises`

```{r g, echo=FALSE}
levels(taxe_df$Goods) %>% kable(caption = 'Modalités de la variable Goods à l\'importation des donnees')
modalites_goods <- length(levels(taxe_df$Goods))
```

La variables **Goods ** a `r modalites_goods ` modalités
...
```{r echo=FALSE,include=TRUE,message=FALSE}
taxe_df <- taxe_df %>% rename(Country_dest=`Country/destination`)
taxe_df$Country_dest <- factor(taxe_df$Country_dest)
levels(taxe_df$Country_dest)
taxe_df$Country_dest <- taxe_df$Country_dest %>% fct_recode(
  "TANZANIE"="TZ",
  "SUEDE"="SWEDEN",
  "RDC"= "RDC/BUNIA",
  "ROYAUME UNI"="R-U",
